...
def reg():
	...
	#----------------------------------------------------------------------------------
	pats.append( re.compile(r"(\w+ ){0,30}((how much)|(quantity)|(measur)?((ement)|(ing)|(e))|(amount)|(mass)) (\w* ){0,4}((vinegar)|(vinigar)|(liquid))\w*") ) #we added this #1
	pats.append( re.compile(r"(\w+ ){0,30}((type)|(kind)|(variety)|(brand)|(form)) (\w* ){0,4}((vinegar)|(vinigar)|(liquid))\w*") )#2
	pats.append( re.compile(r"(\w+ ){0,30}((what)|(wat)|(which)|(wich)|(type of)|(kind of)|(name of)) (\w* ){0,4}((materia)?((l)|(s)))|((sampl)?((es)|(e)))(\w* ){0,4}(test)\w*") )#3
	pats.append( re.compile(r"(\w+ ){0,100}((different)|(four)|(4)) (\w* ){0,10}((samples)|(materials)|(substances))\w*") )#3.1
	pats.append( re.compile(r"(\w+ ){0,30}((size)|(surface area)) (\w* ){0,4}(((materia)?((s)|(ls)))|((sampl)?((e)|(es)))|((elemen)?((t)|(ts)))|((cup)?((p)|(ps))))\w*") ) #4
	pats.append( re.compile(r"(\w+ ){0,100}((long)|(amount)|(time)) (\w* ){0,10}((rinse)|(rinsed))\w*") )#5
	pats.append( re.compile(r"(\w+ ){0,100}((what)|(which)|(type of)|(conditions)) (\w* ){0,10}((drying)|(dry))\w*") )#6
	pats.append( re.compile(r"(\w+ ){0,100}((size)|(type)|(how big)) (\w* ){0,10}((container))\w*") ) #7

	

###We added them till here
